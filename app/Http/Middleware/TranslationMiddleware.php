...
class TranslationMiddleware
{
    ...
    {
        $domain = env('DOMAIN', 'comroads.com');
        ...
        //$uriLocale     = $this->uriLocalizer->getLocaleFromUrl($currentUrl) ? $this->uriLocalizer->getLocaleFromUrl($currentUrl) : 'en';
        $defaultLocale = $this->config->get('app.locale');
        $uriLocale     = subdomain() ?: $defaultLocale;
        ...
        if ($uriLocale) {
            ...
            foreach ($selectableLanguages as $lang) {
                //dd( $currentUrl, $selectableLanguages );
                $altLocalizedUrls[] = [
                    'locale' => $lang->locale,
                    'name'   => $lang->name,
