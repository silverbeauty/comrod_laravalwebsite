...
<h4 class="Content__Heading show-if-there-is-comment {{ $total_comments == 0 ? 'hidden' : '' }}">
    <span>{{ trans('video.other-comments') }} (<span class="total-comments-counter no-padding">{{ $content->total_comments }}</span>)</span>
    <div class="dropdown right">
        {{ $comments_sort_types[$comments_sort_type]['text'] }} &nbsp;&nbsp;<a data-toggle="dropdown" aria-expanded="false"><i class="fa fa-chevron-circle-down fa-lg"></i></a>
                ...
                <div class="media-body">
                    <div class="media">
                        <h4 class="media-heading">
                            <a href="{{ $comment_owner->url }}">{{ $comment_owner->username }}</a>
                            <span class="date-posted">{{ $comment->created_at->diffForHumans() }}</span>
                        </h4>
                        ...
                        <div class="navbar">
                            <ul class="nav navbar-nav">
                                <li>
                                    ...
                                    ><i class="fa fa-retweet"></i> {{ trans('video.reply') }}</a>
                                </li>                            
                                @if ($total_replies)
                                <li>                          
                                    <a 
                                ...
                                <li>
                                    <div>
                                        ...
                                        ><i class="fa fa-thumbs-up"></i></a> <span id="commentLikesCounter-{{ $comment_id }}">{{ $comment->total_likes }}</span>
                                    </div>
                                    <div>
                                        <a
                                            class="btn-ajax"                                        
                                        ...
                                        ><i class="fa fa-thumbs-down"></i></a> <span id="commentDislikesCounter-{{ $comment_id }}">{{ $comment->total_dislikes }}</span>
                                    </div>
                                </li>                           
                            </ul>
                        </div>                    
