            ...
            <div class="media-body">
                <div>
                    <h4 class="media-heading">
                        <a href="{{ $reply_owner->url }}">{{ $reply_owner->username }}</a>
                        <span class="date-posted">{{ $reply->created_at->diffForHumans() }}</span>
                    </h4>
                    <p>{!! linkify($reply->body) !!}</p>
                    <div class="navbar">
                            ...
                            <li>
                                <div>
                                    <a
                                        ...
                                        title="Like"
                                    ><i class="fa fa-thumbs-up"></i></a> <span id="replyLikesCounter-{{ $reply_id }}">{{ $reply->total_likes }}</span>
                                </div>
                                <div>
                                    <a
                                        class="btn-ajax"
                                        ...
                                        data-likes-counter="#replyLikesCounter-{{ $reply_id }}"
                                        title="Dislike"
                                    ><i class="fa fa-thumbs-down"></i></a> <span id="replyDislikesCounter-{{ $reply_id }}">{{ $reply->total_dislikes }}</span>
                                </div>
                            </li>
                        </ul>
                    </div>                    
        ...
        >
            ...
            <div class="media-body">                
                ...
                <div>
                    ...
                    >&nbsp;&nbsp;{{ trans('video.reply') }}&nbsp;&nbsp;</button>&nbsp;&nbsp; {{ trans('video.to') }}
                    <a
                        href="{{ $comment_owner->url }}"
                    >{{ $comment_owner->username }}</a>
                </div>
            </div>
        </form>
    </div>
